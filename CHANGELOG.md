...
- fix(instrumentation): the root path `/` can now be configured as a health check route via `MONOCLE_HEALTH_CHECK_ROUTES`. Route normalization stripped trailing slashes, so `/` collapsed to an empty string and was silently dropped from the configured list and never matched a span — a probe on `/` that answered with a body (a kubernetes `httpGet` probe or a load balancer health check on the root path) was exported as real traffic on every single request. `/` stays out of the defaults because apps commonly serve real traffic there, and opting in never samples out real traffic: it is matched exactly rather than as a suffix, and it is confirmed against the request url, since a route is a pattern relative to whatever the app is mounted under and a sub app mounted at `/api` reports its own root endpoint as route `/`
